 ...
 * 不写文档、reverse=undefined（noop_inverse 在 insertImage helper 的 appendOperation 里）。
 ...
 * - ToolResult.data.base64 只在内存态（ImagePreviewCard 消费），不进 serializeForStorage（NFR-09 路径 C）
 */
...
export const generateWordImageTool: ToolDef = {
  ...
  kind: 'write',
  description: '根据描述生成一张图片，准备插入当前 Word 文档末尾（生成后弹出预览，用户确认后才实际插入）。' +
  ...
  humanLabel: (args) => {
    ...
    return `生成图片（Word）：${prompt.slice(0, 20)}${prompt.length > 20 ? '…' : ''}`;
  },
