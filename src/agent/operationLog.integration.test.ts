...
/**
 * makeLiveMergeMock — 构造「unmerge + range.values=snapshot 真实改变单元格」的 Excel mock（Phase 28 MR-02 守门用）。
 * 旧 mockExcel 的 range.values setter 是 no-op（只调 setValues spy，getter 恒返回 [[0,0]]），
 * 导致「merge undo 还原被丢弃的非左上单元格值」这一数据安全硬门只能断言 rolled_back 路由、
 * 无法断言值真的被写回。本工厂的 values setter 真回写 backing 二维数组（保持引用，仿 makeLiveTable），
 * getter 返回同一引用 → restoreMergeState 走 unmerge → range.values=snapshot 后重读 backing 能反映恢复，
 * 使「只 unmerge 不写回」的回归在测试中可见（断言会红 = 真数据安全守门）。
 * @returns { cells } backing 二维数组引用（供测试断言恢复后的值）+ unmerge/merge spy
 */
function makeLiveMergeMock(initial: unknown[][]): {
  cells: unknown[][];
  unmergeSpy: ReturnType<typeof vi.fn>;
  mergeSpy: ReturnType<typeof vi.fn>;
} {
  const cells: unknown[][] = initial.map((row) => [...row]);
  ...
  const range = {
    ...
    set values(v: unknown[][]) {
      // 真回写：替换 backing 内容（保持引用，仿 makeLiveTable）
      cells.length = 0;
      v.forEach((row) => cells.push([...row]));
    },
    merge: mergeSpy,
    unmerge: unmergeSpy,
  };
  const worksheet = { getRange: () => range };
  (global as unknown as Record<string, unknown>).Excel = {
    run: vi.fn(async (cb: (ctx: unknown) => unknown) =>
      cb({
        // restore_merge_state 的 reverse.args.address 带 Sheet 前缀（'Sheet1!A1:C1'）→
        // resolveRange 走 getItem 分支；不带前缀走 getActiveWorksheet。两路返回同一 range。
        workbook: {
          worksheets: { getActiveWorksheet: () => worksheet, getItem: () => worksheet },
        },
        sync: vi.fn().mockResolvedValue(undefined),
      }),
    ),
  };
  return { cells, unmergeSpy, mergeSpy };
}
...
describe('集成：Phase 28 Excel 工具补全 inverse replay 守门', () => {
  afterEach(() => __resetOperationLogForTest());

  it('单步撤销 merge_cells（merge 路径）：restore_merge_state 收 Record 对象 → rolled_back（adapter 已实现）', async () => {
    mockOfficeSupportsAll();
    mockExcel();
    const adapter = new ExcelAdapter();
    const entry: OperationLogEntry = {
    ...
    const detail = await replayUndoSingle(entry, adapter as unknown as DocumentAdapterForReplay);
    // adapter.restoreMergeState 已实现 → rolled_back
    expect(detail.status).toBe('rolled_back');
  });

  // MR-02 数据安全硬门：merge undo 必须真把「被 merge 丢弃的非左上单元格值」写回（非仅 unmerge）。
  // 用 live values mock（setter 真回写 backing 数组），断言 undo 后 B1/C1 恢复为快照值（非空）。
  // 若有人把 restoreMergeState 退化成「只 unmerge 不写回 values」，此处断言立即变红。
  it('单步撤销 merge_cells（merge 路径）：unmerge 后被丢弃的 B1/C1 值真回写（数据安全硬门，MR-02）', async () => {
    mockOfficeSupportsAll();
    // 初始 = merge 之后的状态：B1/C1 已被 merge 清空（仅左上 A1 保留），模拟「值已丢失」。
    const { cells, unmergeSpy } = makeLiveMergeMock([['标题', '', '']]);
    const adapter = new ExcelAdapter();
    const entry: OperationLogEntry = {
      runId: 'run-28-merge-restore', stepIndex: 0,
      toolName: 'merge_cells',
      args: { address: 'A1:C1', operation: 'merge', across: false },
      humanLabel: '合并单元格 A1:C1',
      reverse: {
        tool: 'restore_merge_state',
        // 快照 = merge 前的完整三格值；undo 须把它写回
        args: { address: 'Sheet1!A1:C1', operation: 'merge', across: false, snapshot: [['标题', 'X', 'Y']] },
      },
      postState: { kind: 'excel_merge', content: { address: 'A1:C1', operation: 'merge' } },
      timestamp: 0,
    ...
    const detail = await replayUndoSingle(entry, adapter as unknown as DocumentAdapterForReplay);
    expect(detail.status).toBe('rolled_back');
    // 硬门 1：先 unmerge（解除合并才能逐格写回）
    expect(unmergeSpy).toHaveBeenCalledTimes(1);
    // 硬门 2：被 merge 丢弃的 B1/C1 值真回写为快照值（非空）——「只 unmerge 不写回」时此处会红
    expect(cells[0][0]).toBe('标题');
    expect(cells[0][1]).toBe('X');
    expect(cells[0][2]).toBe('Y');
  });
  ...
  it('单步撤销 create_pivot_table：delete_pivot_table_by_name 收 Record 对象 → rolled_back（adapter 已实现）', async () => {
    mockOfficeSupportsAll();
    mockExcel();
    const adapter = new ExcelAdapter();
    const entry: OperationLogEntry = {
    ...
    const detail = await replayUndoSingle(entry, adapter as unknown as DocumentAdapterForReplay);
    // Wave 3：adapter.deletePivotTableByName 已实现 → rolled_back
    expect(detail.status).toBe('rolled_back');
  });
