...
<task type="auto">
  ...
  <verify>
    <automated>grep -c 'noop+gate' .planning/phases/08-foundation-a-f/CONTRACT.md && grep -c 'planned' .planning/phases/08-foundation-a-f/CONTRACT.md && npm test -- --run src/agent/contract.test.ts 2>&1 | tail -10</automated>
  </verify>
  ...
  <done>
    ...
    - 合约表行数（grep -v '^#' .planning/phases/08-foundation-a-f/CONTRACT.md | grep -c '| planned\|| done'）≥ 23（注：用 grep -v '^#' 过滤注释行，避免 Nyquist grep gate 误判）
    - WARNING #8 双保险：npm test -- --run src/agent/contract.test.ts 退出 0（含 CONTRACT.length ≥ 23 断言）
  </done>
</task>
