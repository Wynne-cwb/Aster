...
1. **工程隔离（30-D-01 硬约束）：** 无任何 `import` 语句、无 `require()`、不引 `../../src/` 路径、不使用 `Office.*` 任何 API（`Office.run`、`Excel.run` 等）
...
预期：0
  ...
  <acceptance_criteria>
    ...
    - `grep -c 'Office\.' wps-probe/probe.js` 输出 `0`（不用 Office.* API）
  </acceptance_criteria>
...
echo "Office. 出现次数（期望 0）："
grep -c 'Office\.' /Users/wb.chen/Documents/Project/Aster/wps-probe/probe.js || echo "0"
