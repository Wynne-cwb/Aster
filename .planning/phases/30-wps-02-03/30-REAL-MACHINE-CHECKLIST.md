...
> 🔴 **务必用「新建空白文件」跑探针，切勿在含重要数据的文档上运行。**
> 探针会**临时写 Excel 的 B1 单元格**（跑完自动还原原值）、**在 PPT 临时插一张幻灯片**（跑完自动删除）。虽已带还原逻辑，但真机行为尚未最终验证——**请只在新建的空白表格 / 空白演示上运行，彻底排除数据风险。**

- [ ] **新建空白文件备用：** 准备一个新建的空白金山表格（`.et`/`.xlsx`）和一个新建的空白金山演示（`.ppt`/`.pptx`），探针只在这两个空白文件上运行。
...
> 🔴 **再次提醒：两个宿主都用「新建空白文件」运行**（空白表格 + 空白演示）。探针会临时写 Excel B1（跑完还原）、PPT 临时加一张幻灯片（跑完删除），别在含重要数据的文档上跑。
