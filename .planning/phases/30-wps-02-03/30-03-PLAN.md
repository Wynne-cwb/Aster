...
must_haves:
  truths:
    ...
    - "清单含 sideload 安装步骤（路径 A: oem.ini / 路径 B: 联系 Claude 获取方案，无具体 publish.html URL）"
    ...
    - "wps-probe/README.md 含安装步骤（路径 A: oem.ini / 路径 B: 联系 Claude）和探针用途说明"
  artifacts:
...
预期：≥1（收尾回贴步骤）
  ...
  <acceptance_criteria>
    ...
    - 清单路径 B 节描述「截图失败现象发给 Claude」（无具体 publish.html URL，避免 404）
    ...
    - `grep -c '金山表格（ET）.*运行一次\|运行一次.*金山表格（ET）' .planning/phases/30-wps-02-03/30-REAL-MACHINE-CHECKLIST.md` 输出 `≥1`（第 3.5 步两宿主运行步骤）
    - `grep -c '金山演示（WPP）.*运行一次\|运行一次.*金山演示（WPP）' .planning/phases/30-wps-02-03/30-REAL-MACHINE-CHECKLIST.md` 输出 `≥1`（第 3.5 步两宿主运行步骤）
  </acceptance_criteria>
...
预期：≥1（安全说明）
  ...
  <acceptance_criteria>
    ...
    - README.md 路径 B 节描述「联系 Claude 获取方案」，不含具体 publish.html URL
...
# publish.html URL 已从 README 移除（路径 B = 联系 Claude 获取方案）；改为验证「联系 Claude」描述存在
grep -c 'Claude' /Users/wb.chen/Documents/Project/Aster/wps-probe/README.md  # 期望: ≥1（路径 B 联系 Claude 说明）
```
</verification>

<success_criteria>
- `.planning/phases/30-wps-02-03/30-REAL-MACHINE-CHECKLIST.md` 存在，含 go/no-go 裁定框架（精确措辞：「两生死线绿 AND (Excel 基础读写撤销绿 OR PPT 基础读写撤销绿)」）、前置 Key 步骤（30-D-04）、sideload 两条路径（路径 A oem.ini / 路径 B 联系 Claude 获取方案）、两宿主运行步骤（步骤 3.5）、localStorage 手动项图文步骤、DevTools 辅助验证（F12/ALT+F12）、复制结果报告回贴步骤
