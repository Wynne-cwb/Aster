...
注意：Phase 10 的 integration.test 守门**用 vi.fn() mock adapter**（不用真实 ExcelAdapter/PptAdapter）——Wave 0 时 adapter 还没有新方法，用 vi.fn() mock 让骨架能运行。Wave 1-4 实现 adapter 方法后，可选择性地补真实 adapter 路径测试。与 Phase 9 不同（Phase 9 用真 WordAdapter），Phase 10 adapter 方法在 Excel.run/PowerPoint.run 闭包内，需要 Office.js 全局，mock 更简单可靠。每条测试的 toolName 字面量仍是 D-17 硬卡必需品。
</interfaces>
</context>
...
<task type="auto">
  ...
  <read_first>
    - src/agent/operationLog.integration.test.ts（完整文件，重点：imports lines 1-30、mockAdapter 构建范式 lines 193-217、describe 块结构、afterEach 清理）
...
**守门测试策略：** Wave 0 阶段用 vi.fn() mock adapter（不用真实 ExcelAdapter/PptAdapter），原因：Excel.run / PowerPoint.run 闭包需要 Office.js 全局，mock 复杂度不值得在 Wave 0 建立。mock 的 vi.fn() 签名声明为 `async (_args: Record<string, unknown>)` 验证 D-18 Record 签名语义——这与 Phase 9 Word 用真 adapter 的策略差异已有 RESEARCH.md 记录（§Validation Architecture）。
...
describe('集成：replay engine × Phase 10 Excel + PPT 工具守门骨架', () => {
  ...
  it('D-17: format_excel_range → restore_range_format → rolled_back', async () => {
    const fn = vi.fn(async (_args: Record<string, unknown>): Promise<void> => {});
    const entry: OperationLogEntry = {
    ...
    const detail = await replayUndoSingle(entry, { restoreRangeFormat: fn } as DocumentAdapterForReplay);
    expect(detail.status).toBe('rolled_back');
    expect(fn).toHaveBeenCalledTimes(1);
    expect(typeof (fn.mock.calls[0][0] as Record<string, unknown>)).toBe('object');
  });
  ...
  it('D-17: set_column_row_size → restore_column_row_size → rolled_back', async () => {
    ...
    const detail = await replayUndoSingle(entry, { restoreColumnRowSize: fn } as DocumentAdapterForReplay);
    expect(detail.status).toBe('rolled_back');
    expect(fn).toHaveBeenCalledTimes(1);
    expect(typeof (fn.mock.calls[0][0] as Record<string, unknown>)).toBe('object');
  });
  ...
  it('D-17: set_auto_filter → restore_auto_filter → rolled_back', async () => {
    ...
    const detail = await replayUndoSingle(entry, { restoreAutoFilter: fn } as DocumentAdapterForReplay);
    expect(detail.status).toBe('rolled_back');
    expect(fn).toHaveBeenCalledTimes(1);
  });
  ...
  it('D-17: add_conditional_format → restore_conditional_format → rolled_back', async () => {
    ...
    const detail = await replayUndoSingle(entry, { restoreConditionalFormat: fn } as DocumentAdapterForReplay);
    expect(detail.status).toBe('rolled_back');
    expect(fn).toHaveBeenCalledTimes(1);
  });
  ...
  it('D-17: create_table → delete_table_by_name → rolled_back', async () => {
    ...
    const detail = await replayUndoSingle(entry, { deleteTableByName: fn } as DocumentAdapterForReplay);
    expect(detail.status).toBe('rolled_back');
    expect(fn).toHaveBeenCalledTimes(1);
    expect((fn.mock.calls[0][0] as Record<string, unknown>).tableName).toBe('季度数据');
  });
  ...
  it('D-17: freeze_panes → restore_freeze_panes → rolled_back', async () => {
    ...
    const detail = await replayUndoSingle(entry, { restoreFreezePanes: fn } as DocumentAdapterForReplay);
    expect(detail.status).toBe('rolled_back');
    expect(fn).toHaveBeenCalledTimes(1);
  });
  ...
  it('D-17: set_chart_title → restore_chart_title → rolled_back', async () => {
    ...
    const detail = await replayUndoSingle(entry, { restoreChartTitle: fn } as DocumentAdapterForReplay);
    expect(detail.status).toBe('rolled_back');
    expect(fn).toHaveBeenCalledTimes(1);
    expect((fn.mock.calls[0][0] as Record<string, unknown>).chartName).toBe('销售图');
  });
  ...
  it('D-17/D-20: sort_range → restore_range_values_snapshot → rolled_back', async () => {
    ...
    const detail = await replayUndoSingle(entry, { restoreRangeValuesSnapshot: fn } as DocumentAdapterForReplay);
    expect(detail.status).toBe('rolled_back');
    expect(fn).toHaveBeenCalledTimes(1);
    expect((fn.mock.calls[0][0] as Record<string, unknown>).address).toBe('A1:E500');
  });
  ...
  it('D-17: manage_worksheet(add) → restore_worksheet_snapshot → rolled_back', async () => {
    ...
    const detail = await replayUndoSingle(entry, { restoreWorksheetSnapshot: fn } as DocumentAdapterForReplay);
    expect(detail.status).toBe('rolled_back');
    expect(fn).toHaveBeenCalledTimes(1);
  });
  ...
  it('D-17: set_shape_text_font → restore_shape_font → rolled_back', async () => {
    ...
    const detail = await replayUndoSingle(entry, { restoreShapeFont: fn } as DocumentAdapterForReplay);
    expect(detail.status).toBe('rolled_back');
    expect(fn).toHaveBeenCalledTimes(1);
    expect(typeof (fn.mock.calls[0][0] as Record<string, unknown>)).toBe('object');
  });
  ...
  it('D-17: set_shape_text_alignment (spike S4 happy-path) → restore_shape_alignment → rolled_back', async () => {
    ...
    const detail = await replayUndoSingle(entry, { restoreShapeAlignment: fn } as DocumentAdapterForReplay);
    expect(detail.status).toBe('rolled_back');
    expect(fn).toHaveBeenCalledTimes(1);
  });
  ...
  it('D-17: add_shape → delete_shape_by_id → rolled_back', async () => {
    ...
    const detail = await replayUndoSingle(entry, { deleteShapeById: fn } as DocumentAdapterForReplay);
    expect(detail.status).toBe('rolled_back');
    expect(fn).toHaveBeenCalledTimes(1);
    expect((fn.mock.calls[0][0] as Record<string, unknown>).slide_index).toBe(1);
  });

  // ─── PPT noop+gate（D-17 第 4 步：验 skipped_error 路径）───
  ...
  it('D-17: rotate_shape (spike S1 happy-path) → restore_shape_rotation → rolled_back', async () => {
    ...
    const detail = await replayUndoSingle(entry, { restoreShapeRotation: fn } as DocumentAdapterForReplay);
    expect(detail.status).toBe('rolled_back');
    expect(fn).toHaveBeenCalledTimes(1);
  });
  ...
  it('D-17: set_slide_background (spike S2 happy-path) → restore_slide_background → rolled_back', async () => {
    ...
    const detail = await replayUndoSingle(entry, { restoreSlideBackground: fn } as DocumentAdapterForReplay);
    expect(detail.status).toBe('rolled_back');
    expect(fn).toHaveBeenCalledTimes(1);
  });
  ...
  it('D-17: copy_slide → delete_slide_by_index → rolled_back（index+ID 双定位）', async () => {
    ...
    const detail = await replayUndoSingle(entry, { deleteSlideByIndex: fn } as DocumentAdapterForReplay);
    expect(detail.status).toBe('rolled_back');
    expect(fn).toHaveBeenCalledTimes(1);
    expect((fn.mock.calls[0][0] as Record<string, unknown>).capturedId).toBe('slide-uuid-copy');
  });
});
```
...
**RED 状态预期（Wave 0）：** operationLog.ts 已有 15 个新 case（Task 1），但 adapter 实际方法尚不存在（Wave 1-4 实现）→ 测试调用时 adapter.restoreXxx 不存在 → executeReverse 的 `if (!adapter.restoreXxx) throw` 分支 → 等效 skipped_error。Wave 1-4 实现 adapter 方法后这些守门逐步变 GREEN。noop+gate 两条（delete_shape / manage_slides）Wave 0 就 GREEN（noop_inverse case 已存在）。
  </action>
  <verify>
  ...
  <acceptance_criteria>
    ...
    - npm test -- src/agent/operationLog.integration.test.ts 可执行（部分 RED 可接受；noop+gate 两条 GREEN）
...
| integration.test → operationLog | 测试用 vi.fn() mock 验证 reverse.args 传输形状，Wave 0 守门骨架 |

## STRIDE Threat Register

| Threat ID | Category | Component | Disposition | Mitigation Plan |
|-----------|----------|-----------|-------------|-----------------|
| T-10-01 | Tampering | DocumentAdapterForReplay + executeReverse | mitigate | D-18 所有 15 个新 inverse 方法签名 `(args: Record<string, unknown>)`；任何位置签名都会在 integration.test vi.fn() 中静默接受任何 args → Wave 1-4 真 adapter 路径中暴露。Wave 0 骨架预埋 18 个 toolName 字符串（D-17）确保实现完成后 CI 验证路径存在。 |
...
- operationLog.integration.test.ts 含 18 个 Phase 10 toolName 字符串字面量（D-17 预埋）
