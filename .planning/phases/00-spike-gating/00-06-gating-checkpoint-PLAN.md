...
```
  ...
  <!-- 基于 Task 1 用户 checkpoint 选择，验证对应分支。executor 读取用户输入后选择 PROCEED 或 ABORT 分支验证。 -->
  <acceptance_criteria>
    <!-- 公共（两路径均须满足） -->
    ...
    - MANIFEST.md GATING 记录区已更新：`grep -v '^#' .planning/spikes/MANIFEST.md | grep -c 'PASS\|FAIL\|PROCEED\|ABORT'` 返回 ≥ 1

    <!-- PROCEED 路径（用户选 PROCEED 时验证） -->
    - PROCEED: `grep -c '## Decision: PROCEED' .planning/spikes/GATING-REPORT.md` 返回 ≥ 1
    - PROCEED: `ls .planning/spikes/GATING-FAILED-*.md 2>/dev/null | wc -l` 应返回 0（无失败文件）

    <!-- ABORT 路径（用户选 ABORT 时验证） -->
    - ABORT: `grep -c '## Decision: ABORT' .planning/spikes/GATING-REPORT.md` 返回 ≥ 1
    - ABORT: `ls .planning/spikes/GATING-FAILED-*.md 2>/dev/null | wc -l` 返回 ≥ 1（对应失败文件存在）
  </acceptance_criteria>
  <verify>
