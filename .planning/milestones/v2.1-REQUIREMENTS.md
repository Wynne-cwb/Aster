# Requirements Archive: v2.1 从能用到好用

**Archived:** 2026-06-01
**Status:** SHIPPED — 42/42 requirements Complete（三宿主真机 UAT 全 PASS）
...
- [x] **UI-04**: Markdown 整体渲染优化——表格加边框（`.bubble-ai table` border-collapse + cell border，复用 `--border`/`--surface-2`），及列表/代码块一致性
- [x] **UI-05**: 读取工具卡轻量化（无边框、`--text-3` 字色、占位更小、降视觉权重；write 卡不降权）
- [x] **UI-06**: 首屏骨架屏（Office.onReady 前纯 CSS shimmer，不引新库）
...
- Mapped to phases: 42
- Unmapped: 0 ✓
- **Complete: 42/42 ✓**（三宿主真机 UAT 全 PASS，2026-06-01）

---
*Requirements defined: 2026-05-30*
*Last updated: 2026-06-01 — **v2.1 milestone 收官**：42/42 全部 Complete（三宿主真机 UAT 全 PASS + 上线）。收官时修正溯源表 stale 记账：UI-04（表格边框，12-03 已交付）+ UI-06（骨架屏，12-02 `c2840dc` 已交付）从 Pending/未勾 → Complete。*
*Earlier: 2026-05-30 — Traceability table filled by roadmapper（42/42 requirements mapped to Phases 8–12；Phase 13 = UAT/Release 无独立新需求）*
*Earlier: 2026-05-30 — Phase 8 discuss 后修订 3 条 NFR/PREF（08-CONTEXT D-05/D-08/D-18）：NFR-07 硬 gate→软提醒、PREF-02 偏好上限 200→500、NFR-08 去掉 toolDefs ≤15KB token 门。项目原则「质量 >> 成本&包体积」确立。*
