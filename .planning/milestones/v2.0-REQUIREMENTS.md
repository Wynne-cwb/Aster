# Requirements Archive: v2.0 Office 智能代理
...
- [x] **AGENT-02**：`max_steps = 20` 硬上限不可绕过（fail-safe）；hit 20 时**软着陆**——不直接 abort，而是 push 一条「Aster 觉得这事还没干完，要继续吗？」让用户决定继续 / 停止 / undo all（Phase 3 `loop.ts` MAX_STEPS=20 + `pushSoftLanding`，SC2 PASS）
- [x] **AGENT-07**：跑完后 `<DiffLogPanel/>` 展示 N 步卡片——每条用 `humanLabel(args)` 中文人话（如「在第 3 张幻灯片后插入新幻灯片」），不是 raw tool name
- [x] **AGENT-08**：每个 tool 必须 export `humanLabel(args) => string`，缺则 TS 编译失败（lint/type 强制）（Phase 5 `ToolDef`/`operationLog` 必填字段，dual-path push 用 humanLabel）
...
- [x] **AGENT-13**：单一 `AgentSession.abort(reason)` 入口统一 4 路 abort 信号：visibility / user pause / max_steps / circuit breaker（Phase 3 `agentStore.ts` `AbortReason = 'visibility'|'user'|'max_steps'|'circuit'`）
...
- [x] **ERR-01**：Tool error 结构化 schema = `{ code: enum, message: zh-CN, recoverable: boolean, hint: string }`，code 枚举至少含 `INVALID_ARGS / NOT_FOUND / PERMISSION_DENIED / HOST_API_FAILED / CIRCUIT_OPEN / STEP_LIMIT / UNSUPPORTED`（Phase 3 `ToolErrorCode` enum @ `tools/index.ts`，SC3 PASS）
- [x] **ERR-02**：Tool error 经 sanitization 后才回灌给 LLM——禁止把内部状态（文件路径 / Key 片段 / stack trace）写进 message（Phase 3 `sanitizeFromAsterError` 唯一边界，不读 err.stack，SC3 PASS）
...
- [x] **TOOL-03**：Write tools P1 — PPT: `insert_slide` / `set_shape_text` / `set_shape_property` / `move_shape` / ~~`insert_image_on_slide`（聚合 v1 F4 多模态）~~ / Excel: `set_range_values` / `apply_formula` / `insert_chart` / `set_cell` / Word: `insert_paragraph` / `replace_paragraph` / `insert_text_at_cursor` / `replace_selection`
  > **收官准确性修订（2026-05-30）：** 除 `insert_image_on_slide` 外 13 个 write tool 全部实现并真机 UAT PASS；`insert_image_on_slide`（图片生成插入）经 Phase 6 Out-of-scope 列为「v2.0 stretch 不进 P1」**实际未实现**——aihubmix 生图客户端代码在基座但未接 agent、无此 tool、无 UI。该子项移交 v2.1 **FUT-16**（见 PROJECT.md Active）。TOOL-03 主体（其余 13 tool）判 Complete 成立。
...
- [x] **CARRY-01**：FU-01 首次取选区 bug 修复——必须在 Phase 3 read tools 上线前修，否则后续所有 read tool 都受污染（Phase 3 commit `b701fcd` path A：main.tsx Office.onReady 预取 + useSelectionStore，SC4 PASS）
...
- [~] **ONB-01**：~~Onboarding 第二步必须包含动画 / GIF 示意「跑完会这样汇报」~~ — **DESCOPED (v2.0)**：Phase 6 决策 D-18/D-19 把 Onboarding 收成单步、删除 `Step2Guide.tsx`，原「第二步 GIF」承载位消失。心智锚定改由 empty-state killer-scenario chips（ONB-03）+ 全程中文 humanLabel step 摘要（ONB-02）承担。GIF/动画推迟到 v2.1 重新评估（早期用户 = 作者本人 + 亲人，教学成本可控）。
...
- [x] **NFR-02**：初始 JS ≤ 1MB gzipped；v2 实测目标 ~70KB（继承 v1 N2，0 净新增运行时依赖）（实测 73.42 KB gzip ≤ 82 KB CI gate ≪ 1MB；0 净新增运行时依赖达成）
...
- **FUT-13**（← ONB-01 descoped v2.0）：Onboarding 心智锚定动画 / GIF「跑完会这样汇报」——v2.0 收成单步 Onboarding 后承载位移除；扩用户范围后重做（中文用户对「AI worker」无心智锚定，教学成本最贵）
...
| CARRY-01 | Phase 3 | Complete |
| CARRY-02 | Phase 4 | Complete |
| CARRY-03 | Phase 5 | Complete |
| ONB-01 | Phase 6 | Descoped (→ v2.1) |
...
**Coverage (final, v2.0 close 2026-05-30):**
- v2.0 requirements: 31 total (26 functional + 5 NFR)
- Delivered: 30/31 ✓ (全部 code-level 验证 + 三宿主真机 UAT 全 PASS)
- Descoped: 1 — **ONB-01**（Onboarding GIF/动画）经 Phase 6 D-18/D-19 主动砍掉，承载位随单步 Onboarding 移除；推迟 v2.1
- Unmapped: 0 ✓
