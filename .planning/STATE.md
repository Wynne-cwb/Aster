...
stopped_at: "**v2.4「扩疆域」里程碑启动（2026-06-05，`/gsd-new-milestone`）。** 三条线：C 工具补全（三宿主高价值 write 工具 ~11：Word 高亮/列表/批注/页眉页脚/edit_table · Excel 合并/去重/数据透视 · PPT 插表格/线条箭头/渐变）+ D WPS 兼容（WPS Windows 桌面版 spike-gate 两层探路：调研层 Claude 跑 + 真机层用户 Windows/WPS 跑，只出可行性裁定）+ 配置导入导出（明文 JSON 文件搬家，含 API key，醒目警告，复用 v2.2 FILE 基建）。PROJECT.md 已更新（Current Milestone v2.4 段 + Current focus + Active + footer）。Phase 编号从 25 续接。**下一步：定义需求 → 创建 roadmap → `/gsd-discuss-phase 25`。**"
last_updated: "2026-06-05T09:00:00.000Z"
last_activity: 2026-06-05 -- v2.4 milestone started (defining requirements)
progress:
  total_phases: 0
  completed_phases: 0
  total_plans: 0
...
See: .planning/PROJECT.md (updated 2026-06-05 — Milestone v2.3「精装与定力」shipped)

**Core value:** 在原生 Office 内部，让中文职场用户用自带 API Key 享受 **AI 代理** 能力，能完成绝大部分文档工作；无后台、BYO Key。
**Current focus:** **v2.3「精装与定力」已发布并归档（2026-06-05，tag `v2.3`，线上 `1fe9529`）。** 5 phase（20-24）/ 10 plans / 13 需求全交付；A 系列（PPT 视觉质量纵深：设计 token + 几何自查 + apply_slide_layout 盖印章 + 6 版式库 + 自渲染预览自查闭环）+ B 系列（上下文/抗幻觉：时钟脱前缀 + token 水位摘要压缩）；三宿主真机 UAT 全过（11 个真机 bug 全修），PVQ-06 spike-gate 真机判**铺开**。**下一步：`/gsd-new-milestone` 启动下一里程碑（候选：C 工具补全广度 triage / D WPS 兼容）。**

## Current Position

Phase: Not started (defining requirements)
Plan: —
Status: Defining requirements — v2.4「扩疆域」milestone started
Last activity: 2026-06-05 — Milestone v2.4 started (PROJECT.md/STATE.md updated; REQUIREMENTS + roadmap next)

### v2.4 Scope (3 lines — roadmap pending)

| Line | 内容 | 性质 |
|------|------|------|
| **C 工具补全** | 三宿主高价值 write 工具 ~11（Word 高亮/列表/批注/页眉页脚/edit_table · Excel 合并单元格/去重/数据透视 · PPT 插表格/线条箭头/渐变填充）| 确定性主交付 |
| **D WPS 兼容** | WPS Windows 桌面版 spike-gate 两层探路（调研层 Claude + 真机层用户 Windows/WPS）| 押注·只出可行性裁定 |
| **配置导入导出** | 明文 JSON 文件下载/上传搬家（含 API key + Provider + 偏好 + 主题 + Pexels key，醒目警告）| 可移植·复用 v2.2 FILE 基建 |

> Phase 编号从 **25** 续接（默认不 reset）。REQUIREMENTS.md + ROADMAP.md 正由本 `/gsd-new-milestone` 重建。完整范围/取舍见 PROJECT.md §Current Milestone: v2.4 扩疆域。

### v2.4 工程约束（贯穿所有 phase）

- **Bundle gate**: 初始 main bundle ≤82KB gzip CI gate（v2.3 收于 81.3KB，**余量仅 ~0.7KB——很紧**）；新功能/重模块必须懒加载；动 bundle 前先 `npm run build` 再 `npm run size`（陈旧 dist 给假绿，memory `project_bundle_size_guard`）
- **新 write 工具合约（C 线）**: inverse 方法收 Record 对象（非位置参，Phase 5 教训，memory `adapter_inverse_signature`）+ 新 PostStateSnapshot kind + humanLabel + `operationLog.integration.test` 守门 + 入 `*_TOOLS` Set（casing 归一化，memory `project_ppt_officejs_gotchas`）
- **配置导出安全（配置线）**: 明文 JSON + 醒目警告（用户拍板）；Key 导出到用户自控本地文件 ≠ 上传 Aster 服务器，不违反无后台 / Key 不离开浏览器硬约束
- **WPS spike-gate（D 线）**: 真机验证层归用户（macOS 跑不了，需另备 Windows+WPS）；调研层 Claude 先行；**spike 不通过仅交付 C+配置，里程碑仍干净 ship**
- **项目原则**: AI 生成质量 >> token 成本 & 包体积；undo 守门 / bundle gate / P95 仍硬卡

> v2.3 收尾快照（phase 20–24 全 PASS、1075 tests、81.3 KB、三宿主真机 UAT 全过、tag `v2.3`、线上 1fe9529）已归档至 `.planning/milestones/v2.3-ROADMAP.md` + `MILESTONES.md`。
...
- [Phase ?]: html2canvas@1.4.1 exact version (no caret) per T-24-03 supply-chain gate
- [Phase ?]: jsdom downgraded ^29.1.1→^25.0.1: Node 20.17 incompatible with jsdom@29 (requires ^20.19.0)
- [Phase ?]: Wave 0 stubs use describe.skip pattern (not dynamic import) per STATE L112 precedent
...
| Phase 24 P01 | 25 | 3 tasks | 6 files |
| Phase 24-a-p2-bundle P24-04 | 40min | 3 tasks | 5 files |
| 260604-fzn | 修复 UAT-1：apply_slide_layout 真机 ok=false 根因 = 非法 GeometricShapeType 'RoundedRectangle'→'RoundRectangle'（ppt-layouts ShapeSpec 联合+kpi_value；add_shape enum 同类 + 裸 'Arrow'→'RightArrow'；slide-preview 消费端同步）；applySlideLayout 失败时事务性删孤儿页（复用 deleteSlideByIndex 双定位，best-effort 不掩盖原错）；结构守门关 mock-vs-real gap（ppt-layouts.test 全量合法集 a/b/c + satisfies 编译期穷举、强化 addGeometricShape mock 抛非法、PptAdapter.test 动态清理验证）。tsc 0 / 999→1004 passed,0 failed / undo 39 绿 / bundle 80.86KB<82。⚠️ 真机 ok=false 待用户 Office for Web 重测 ① 确认 | 2026-06-04 | 1ecc05f / c878c6d / 0ffad97 | [260604-fzn-fix-uat-1-apply-slide-layout-invalid-geo](./quick/260604-fzn-fix-uat-1-apply-slide-layout-invalid-geo/) |
| 260604-gld | 诊断增强（关 2 个盲区，让下一份调试报告自证版本+真实错误）：① 构建版本戳——vite define `__BUILD_COMMIT__`(git rev-parse --short HEAD, try/catch→unknown)+`__BUILD_TIME__`(ISO)，新建 vite-env.d.ts 声明，buildEnvSection 顶部加 `- build: <commit> @ <time>`（typeof 守卫），判用户是否跑缓存旧 bundle；② 透出工具失败原因——buildChatSection tool ok=false 加 `| error: <code> <message>`(+hint)、buildStepLog 加`- 错误/- 提示`，源 toolResult.error，全程 redactKey；③ HostApiError 加 debugCause(仅 cause.message 截 300，不挂 result)+PptAdapter.applySlideLayout console.warn 真实 Office.js 原因到 DevTools。DEFER「报告内展示 debugCause」（报告数据源=LLM wire 同一 ToolResult，被 ERR-02 隐私门硬禁宿主内部，需新侧信道超 quick 范围）。tsc 0 / 1004→1013 passed,0 failed / 隐私门全绿(ERR-02+T-vtc) / bundle 80.91KB<82。⚠️ 真机价值在用户下一份报告 | 2026-06-04 | 93ad314 / 756eef4 / cb174a3 | [260604-gld-diagnostics-build-version-stamp-tool-fai](./quick/260604-gld-diagnostics-build-version-stamp-tool-fai/) |

## Deferred Items

Items acknowledged and deferred at **v2.3 milestone close on 2026-06-05** (artifact audit `audit-open`，26 项)。用户选「继续关账（确认陈旧项）」。经逐项核对：**全部为陈旧簿记或已被里程碑级 UAT 覆盖，0 真正未完成的工作**（v2.3 已三宿主真机 UAT 全过 + 上线 tag v2.3 @ 1fe9529）。

| Category | Item | Status |
|----------|------|--------|
| debug | ppt-list-slides-host-fail / reasoning-content-roundtrip | 均 fix-applied + 2026-05-29 已部署；状态位未翻，实际已解（v2.0/v2.1/v2.2 已确认） |
| quick_task | 16 项（260527-o8j/opp/q1c · 260529-vtc · 260530-b7s/c14 · 260531-b5o/bg2/l4z/l7v/m4x · 260601-dul/ki6 · 260603-fx8 · **260604-fzn / 260604-gld**） | 均已完成有 commit（见上方 Quick Tasks 表）；目录缺 status 文件的扫描器怪癖。v2.3 新增 2 项 260604-fzn（UAT-1 修复）/ 260604-gld（诊断增强）已交付 |
...
| uat_gap | 19-UAT-PACKET | 属 v2.2，真机 UAT 实测全 PASS，`open_scenario_count: 0`，状态位未翻 |
| uat_gap | 24-UAT-PACKET | 本里程碑 v2.3，统一 UAT 全过（11 个真机 bug 全修 + spike-gate 判铺开），`open_scenario_count: 0`，状态位未翻 |

> **第五次复发提醒：** stale-checkbox / status 字段缺失 / uat 状态位未翻 已是跨 v1.0/v2.0/v2.1/v2.2/**v2.3** 五次 milestone close 的确定模式（memory `recurring_failure_add_gate` / `project_gsd_tooling_quirks`）。「同一故障 ≥2 次加结构性守门」原则在 GSD 收尾簿记上至今未兑现——**确定待还债**（候选守门：close 前自动核对 quick task status 文件 + 翻 traceability checkbox 的脚本）。
